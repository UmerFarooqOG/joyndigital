...
const Page = ({ params }) => {
  ...
  const motionProps = {
    initial: isMobile ? { x: 0, scale: 1 } : { x: -200, scale: 1 },
      ...
      {loader ? (
        ...
        <div className='overflow-hidden w-full mx-auto'>
          <section className='max-w-[1440px] w-full mt-[80px] px-[20px] sm:px-[60px] lg:px-[90px] xlb:px-[120px] py-[2rem] mx-auto'>
            <motion.h5 {...motionProps} className='text-[#2958A5] text-[20px] xsm:text-[40px] font-[500] font-Noto'>{name}</motion.h5>
            <motion.h2 {...motionProps} className='text-[24px] ssm:text-[28px] xsm:text-[40px] sm:text-[52px] lg:text-[62px] xll:text-[72px] leading-normal sm:leading-[100px] w-full text-[#2E2E2E] font-Montserrat font-bold'>{heading}</motion.h2>
            <div className='w-full flex-col my-[30px] flex items-end justify-start sm:justify-end'>
              <div className=' w-full mdd:w-[60%]'>
                <motion.p {...motionProps} className='text-[#7F7F7F] font-Noto text-[17px] xsm:text-[18px] lg:text-[20px] xxl:text-[24px] leading-normal sm:leading-[34px]'>{heroP}</motion.p>
              </div>
            </div>
          </section>

          <section className='max-w-[1440px] w-full my-[20px] sm:my-[80px] h-full flex items-center justify-center flex-col gap-[40px] px-[20px] sm:px-[60px] lg:px-[90px] xlb:px-[120px] py-[2rem] mx-auto'>
          ...
          <section className='max-w-[1440px] w-full my-[20px] sm:my-[80px] h-full flex items-center justify-center flex-col gap-[40px] px-[20px] sm:px-[60px] lg:px-[90px] xlb:px-[120px] py-[2rem] mx-auto'>
            <motion.div {...motionProps} className='w-full flex flex-col sm:flex-row  gap-[10px] mdd:gap-[30px] items-center justify-center sm:items-start'>
              <h5 className='text-[#2E2E2E] font-Montserrat font-[500] flex-1 text-[20px] xsm:text-[30px] sm:text-[25px] mdd:text-[35px] lg:text-[40px] mt-[-10px] lg:mt-[-20px]  xll:text-[48px]'>
            ...
            <motion.div {...motionProps} className='w-full flex flex-col sm:flex-row  gap-[10px] mdd:gap-[30px] items-center justify-center md:justify-end sm:items-start'>
              <motion.div {...motionProps} className='flex flex-wrap mt-[40px] py-[30px] items-center gap-[30px] mdd:gap-[40px] xll:gap-[60px] w-full text-center sm:text-left sm:w-[65%]  lg:w-[60%] xll:w-[65%]'>
                {useLogos.map((i, index) => (
                  <p key={index} className='projDetailLogo'>{i.logo}</p>
                ))}
              </motion.div>
            </motion.div>
          </section>
          ...
          <section className='w-full py-[80px] h-full flex items-center justify-center flex-col gap-[0px] px-[20px] sm:px-[60px] lg:px-[90px] xlb:px-[120px] bg-[#FFFAFA] mx-auto'>
            <div className='max-w-[1440px] mx-auto'>
              ...
              <motion.div {...motionProps} style={bglg === "bluelg" ? blueGradient : redGradient} className='flex flex-col mdd:flex-row  overflow-hidden rounded-[30px] h-full mdd:h-[500px] xxl-h-[520px] w-full relative place-content-center items-center justify-start'>
