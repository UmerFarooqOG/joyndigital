  ...
  return (
    <motion.div initial={initialAnimation} whileInView={whileInViewAnimation} transition={transition} className='flex flex-col mdd:flex-row-reverse my-[20px] rounded-[30px] h-auto  mdd:h-[420px] lg:h-[490px] xlg:h-[510px]  xll:h-[490px]  w-full relative items-center justify-center'>
      <div className='rounded-se-[20px] mdd:rounded-s-[0px] rounded-ss-[20px] mdd:rounded-e-[30px]   h-full w-full mdd:w-[45%] flex flex-col gap-[16px] px-[20px] xsm:px-[20px] xlg:px-[30px] xll:px-[55px] py-[20px] items-start justify-center ssm:py-[60px]  bg-white '>
        ...
        <h3 className='text-[15px] leading-normal mdd:leading-[38px] ssm:text-[18px] xsm:text-[22px] lg:text-[28px] font-normal font-Montserrat'><span className='text-[#EB2022] font-bold'>{spanTitle} </span>{title}</h3>
      ...
      <div style={blueGradient} className='overflow-hidden flex items-center justify-center h-full rounded-se-[0px] mdd:rounded-e-[0px] rounded-es-[20px] rounded-ee-[20px] mdd:rounded-s-[30px]  px-[20px] xsm:px-[30px] xll:px-[70px] py-[20px] xsm:py-[40px] xll:py-[80px] bg-l w-full mdd:w-[55%] lg:w-[60%] '>
        <Image src={image} loading='lazy' width={500} height={500} className='w-full h-full ssm:h-auto lg:w-[700px] lg:h-[366px] object-contain' alt='product image' />
      </div>
    </motion.div>
  );
};
